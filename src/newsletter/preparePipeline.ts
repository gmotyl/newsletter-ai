...
import { displaySuccess, displayInfo, displayProgress, displayVerbose } from "../cli/utils/index.js";
 ...
 */
const cleanAndEnrichLinks = async (state: CollectedNewsletters): Promise<CollectedNewsletters> => {
  ...
  const enrichedNewsletters = await Promise.all(
    state.newsletters.map((newsletter, index) =>
      limit(async () => {
        const urls = state.urls[index] || [];
        const articles: Article[] = [];

        displayVerbose(`\nProcessing ${newsletter.pattern.name} (${urls.length} links)...`);
        ...
        for (const url of urls) {
          ...
          try {
            ...
            if (category === 'bonus') {
              ...
              const resolved = await resolveUrlWithCache(url, 'redirect', undefined, 2, state.config.appConfig.scraperOptions);
            ...
            if (finalCategory !== 'keep') {
              ...
              displayVerbose(`  ✗ Filtered after redirect (${finalCategory}): ${finalUrl}`);
              continue;
            }
            ...
            try {
              ...
              displayVerbose(`  ⚠ Failed to fetch title for ${finalUrl}, using fallback`);
            }
            ...
            stats.errors++;
            displayVerbose(`  ✗ Error processing ${url}: ${error instanceof Error ? error.message : String(error)}`);
            // Skip problematic links rather than failing the entire process
          }
        }

        displayVerbose(`  → Kept ${articles.length}/${urls.length} links`);
 ...
 */
function categorizeLink(url: string): 'keep' | 'sponsored' | 'social' | 'tracking' | 'bonus' | 'youtube' {
  try {
    const urlObj = new URL(url);
    ...
    if (pathname.includes('/open/') || pathname.includes('/click/') || pathname.includes('/track/')) {
      return 'tracking';
    }
    ...
    const bonusKeywords = ['/ebook', '/whitepaper', '/guide', '/download', '/almanac', '/report'];
    if (bonusKeywords.some(keyword => pathname.includes(keyword))) {
      // Additional check: if it's a direct resource (not a landing page)
      if (pathname.endsWith('.pdf') || pathname.includes('/ebooks/') || pathname.includes('/almanac/')) {
        return 'bonus';
      }
    }
    ...
    if (sponsoredDomains.some(domain => hostname.includes(domain))) {
      ...
      if (pathname.includes('/courses/') || pathname.includes('/learn/') || pathname.includes('/course/')) {
        return 'sponsored';
      }
    }

    // Sponsored query params (ref links, affiliate links)
    const sponsoredParams = ['ref=', 'utm_source=email', 'utm_medium=', 'affiliate', 'aff='];
    if (sponsoredParams.some(param => search.includes(param))) {
      // If has ref= or utm_source=email with utm_medium, likely sponsored
      if (search.includes('ref=') || (search.includes('utm_source=email') && search.includes('utm_medium='))) {
        return 'sponsored';
      }
    }
    ...
    if (search.includes('sale') || search.includes('promo') || search.includes('discount') || search.includes('coupon')) {
      return 'sponsored';
    }
...
function isMarketingLink(url: string): boolean {
  const category = categorizeLink(url);
  return category === 'sponsored' || category === 'tracking' || category === 'social';
}

/**
 * Save collected newsletters to LINKS.yaml
 */
const saveToYaml = async (state: CollectedNewsletters): Promise<CollectedNewsletters> => {
  // Save to LINKS.yaml
  await saveLinksToYaml(state.newsletters, 'LINKS.yaml');

  // Calculate totals
  const totalLinks = state.newsletters.reduce((sum, n) => sum + n.articles.length, 0);
  ...
  displayInfo('  2. Run "npm run generate" to process the links and generate summaries');
