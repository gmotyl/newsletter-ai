...
export const searchAndCollectNewsletters = async (
  ...
  if (messageLimit && messageLimit > 0 && totalFound > messageLimit) {
    displayVerbose(
      `\nApplying global message limit: Processing ${messageLimit} of ${totalFound} emails (${totalFound - messageLimit} will be processed in next run)`
    );
    searchSpinner.update(
  ...
  for (const { email, pattern } of emailsToProcess) {
    try {
      ...
      displayError(
        `Failed to process email ${email.subject}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
